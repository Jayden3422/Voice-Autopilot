...
def normalize_text(raw: str) -> str:
  text = raw.replace("，", ",").replace("。", ",") # 统一中文标点
...
def parse_explicit_date(expr: str, now: datetime) -> date | None:
  ...
  # dateparser星期
  settings = {
    "PREFER_DATES_FROM": "future", # 下周的星期三
    "RELATIVE_BASE": now, # 当前时间基准
...
def extract_date(text: str, now: datetime) -> date:
  ...
  1. N天后 / N天之后
  2. 今天 / 明天 / 后天 / 大后天 / 明早
  3. parse_explicit_date 星期几 / 具体月份日期
  4. 解析失败默认今天
  """
...
def extract_time_range(text: str) -> tuple[time, time, str]:
  """
  @return: (start_time, end_time, time_expr)
  time_expr: 语言时间表达
  """
  # 点至点
  ...
  if m:
    ...
    def hour_with_minute(
      h: int,
      minute_str: str | None,
      minute_cn_str: str | None,
      half: str | None
    ) -> tuple[int, int]:
      if minute_str:
...
def parse_calendar_command(raw_text: str, now: datetime | None = None) -> CalendarCommand:
...
if __name__ == "__main__":
  ...
  test_cases = [
    ...
    "今天晚上八点看电影两个小时",
    "帮我添加一个日程，明天十点开会",
    "给Google日历新增日程，后天下午三点和客户开会一个半小时",
    "后天和小王吃饭",
  ]
  for text in test_cases:
    cmd = parse_calendar_command(text, now=now)
    print("原始文本:", text)
    print("  日期:", cmd.date)
