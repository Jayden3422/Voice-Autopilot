...
| 启动预热池 | 后台预初始化：Whisper JIT、Piper ONNX（代表性长度句子）、OpenAI HTTPS 连接池、FAISS 索引 | 消除首次请求延迟；ONNX 执行计划按真实分段长度预编译 |
...
| `piper_tts_zh` | ONNX 执行计划 + g2pW BERT 模型（~16 字句子） | ~5–15 秒 |
| `piper_tts_en` | ONNX 执行计划（~16 字句子） | ~1–3 秒 |
| `openai_api` | httpx 连接池 + 两个客户端单例 | ~0.5–1 秒 |
| `rag_faiss` | FAISS 索引加载入内存（未构建则跳过） | ~1–3 秒 |
